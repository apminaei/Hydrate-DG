...
#include "../dune/Hydrate-DG/Ex1/include_problem.hh"
...
int main(int argc, char **argv)
{
	try
	{
		...
		if (argc != 2)
		{
			if (helper.rank() == 0)
			{
				std::cout << "usage: ./ex1 <input_file.ini> " << std::endl;
			}
			return 1;
		}
		std::string PATH = "/home/peiravim/dune-2.7/Hydrate-DG/dune/Hydrate-DG/Ex1/";
		char input[80];
		sscanf(argv[1], "%39s", input);
		std::string input_file = "/home/peiravim/dune-2.7/Hydrate-DG/dune/Hydrate-DG/Ex1/inputs/";
		input_file += input;
		...
		*/
		...
		// std::string filename = ptree.get("grid.name",(std::string)"grid.msh");
		// auto grid_file = PATH;
		// grid_file += "grids/";
		// grid_file += filename;
		...
		std::shared_ptr<Grid> grid = Dune::StructuredGridFactory<Grid>::createCubeGrid(ll, ur, elements);
		// load balance the grid
		// std::string filename = ptree.get("grid.alugrid.name",(std::string)"grid.msh");
		// auto grid_file = PATH;
		// grid_file += "grids/";
		// grid_file += filename;
		// Dune::GridFactory<Grid> factory;
