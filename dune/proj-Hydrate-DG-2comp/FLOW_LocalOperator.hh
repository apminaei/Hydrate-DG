...
template <class GV, typename U, class GFS, class FEM_P, class FEM_S, class FEM_T>
class FLOW_LocalOperator : public Dune::PDELab::NumericalJacobianApplyVolume<FLOW_LocalOperator<GV, U, GFS, FEM_P, FEM_S, FEM_T>>,
                           public Dune::PDELab::NumericalJacobianVolume<FLOW_LocalOperator<GV, U, GFS, FEM_P, FEM_S, FEM_T>>,
                           public Dune::PDELab::NumericalJacobianApplySkeleton<FLOW_LocalOperator<GV, U, GFS, FEM_P, FEM_S, FEM_T>>,
                           public Dune::PDELab::NumericalJacobianSkeleton<FLOW_LocalOperator<GV, U, GFS, FEM_P, FEM_S, FEM_T>>,
                           public Dune::PDELab::NumericalJacobianApplyBoundary<FLOW_LocalOperator<GV, U, GFS, FEM_P, FEM_S, FEM_T>>,
                           public Dune::PDELab::NumericalJacobianBoundary<FLOW_LocalOperator<GV, U, GFS, FEM_P, FEM_S, FEM_T>>,
...
public:
  ...
  FLOW_LocalOperator(const GV &gv_,
                     ...
                     double alpha_g_ = 1., double alpha_w_ = 1., double alpha_s_ = 1., double alpha_T_ = 1.)
      : gv(gv_),
        unew(unew_),
        gfs(gfs_),
        time(time_),
        dt(dt_),
        intorder(intorder_),
        method_g(method_g_), method_w(method_w_), method_T(method_T_),
        alpha_g(alpha_g_), alpha_w(alpha_w_), alpha_s(alpha_s_), alpha_T(alpha_T_),
        cache_Pg(20), cache_Pc(20), cache_Sw(20), cache_Sh(20), cache_T(20)
  {
    theta_g = 1.0;
  ...
  void alpha_volume(const EG &eg, const LFSU &lfsu, const X &x, const LFSV &lfsv, R &r) const
  {
    ...
    for (const auto &ip : quadratureRule(geo, intorder))
    {
      ...
      auto Diffcoeff_m = DH2O_g * rho_g * Sg * (-YCH4 + H_M_w / (H_M_w - zCH4 * P_H_sat)) / Pg + DCH4_w * rho_w * Sw * zCH4 / (H_M_w - zCH4 * P_H_sat);
      //Water
      ...
      for (size_type i = 0; i < lfsv_Pg.size(); i++)
      {
        r.accumulate(lfsv_Pg, i, ((Concoeff_m_g * (Kgradu_Pg * gradpsi_Pg[i]) + Concoeff_m_w * (Kgradu_Pg * gradpsi_Pg[i] - Kgradu_Pc * gradpsi_Pg[i])) - (Diffcoeff_m * (gradu_Pg * gradpsi_Pg[i])) - q_g * psi_Pg[i]) * factor);
      }
      // rho_w * krW * (Kgradu_Pg * gradpsi_Pc[i] - Kgradu_Pc * gradpsi_Pc[i])
      for (size_type i = 0; i < lfsv_Pc.size(); i++)
      {
        r.accumulate(lfsv_Pc, i, ((Concoeff_w_g * (Kgradu_Pg * gradpsi_Pc[i]) + Concoeff_w_w * (Kgradu_Pg * gradpsi_Pc[i] - Kgradu_Pc * gradpsi_Pc[i])) - (Diffcoeff_w * (gradu_Pg * gradpsi_Pc[i])) - q_w * psi_Pc[i]) * factor);
      }
      ...
      for (size_type i = 0; i < lfsv_T.size(); i++)
      {
        r.accumulate(lfsv_T, i, (rho_g * krN * (Kgradu_Pg * gradpsi_T[i]) * Cp_g * T + rho_w * krW * (Kgradu_Pg * gradpsi_T[i] - Kgradu_Pc * gradpsi_T[i]) * Cp_w * T + kth_eff * (gradu_T * gradpsi_T[i]) - Q * psi_T[i]) * factor);
      }
    } //End Quadrature Rule
  }   // End Alpha_volume
                      ...
                      R &r_s, R &r_n) const
  {
    ...
    for (const auto &ip : quadratureRule(geo, intorder))
    {
      ...
      double term_conv_m_w = -krW * ( omega_s * rho_w_s * XCH4_s * ((Kgradu_Pg_s * n_F_local) - (Kgradu_Pc_s * n_F_local)) 
                                    + omega_n * rho_w_n * XCH4_n * ((Kgradu_Pg_s * n_F_local) - (Kgradu_Pc_s * n_F_local)));
      double term_diffusion_m = term_conv_m_g + term_conv_m_w + DH2O_g * ( omega_s * Diffcoeff_m_g_s * (gradu_Pg_s * n_F_local) 
                          + omega_n * Diffcoeff_m_g_s * (gradu_Pg_n * n_F_local))
                          + DCH4_w * (omega_s * Diffcoeff_m_w_s * (gradu_Pg_s * n_F_local) 
                          + omega_n * Diffcoeff_m_w_s * (gradu_Pg_n * n_F_local )) ;
      ...
      for (size_type i = 0; i < lfsv_Pg_s.size(); i++)
      {
        r_s.accumulate(lfsv_Pg_s, i, term_nipg_g * krN * omega_s * rho_g_s * (Kn_F_s * gradpsi_Pg_s[i]) * factor);
      }
      for (size_type i = 0; i < lfsv_Pg_n.size(); i++)
      {
        r_n.accumulate(lfsv_Pg_n, i, term_nipg_g * krN * omega_n * rho_g_n * (Kn_F_n * gradpsi_Pg_n[i]) * factor);
      }
      ...
      //Water
      auto Diffcoeff_w_g_s =  - Diffcoeff_m_g_s;
      auto Diffcoeff_w_g_n =  - Diffcoeff_m_g_n;

      auto Diffcoeff_w_w_s =  - Diffcoeff_m_w_s;
      auto Diffcoeff_w_w_n =  - Diffcoeff_m_w_n;
      ...
      double term_conv_w_w = -krW * ( omega_s * rho_w_s * XH2O_s * ((Kgradu_Pg_s * n_F_local) - (Kgradu_Pc_s * n_F_local)) 
                                    + omega_n * rho_w_n * XH2O_n * ((Kgradu_Pg_s * n_F_local) - (Kgradu_Pc_s * n_F_local)));
      double term_diffusion_w = term_conv_w_g + term_conv_w_w + DH2O_g * (omega_s * Diffcoeff_w_g_s * (gradu_Pg_s * n_F_local) 
                          + omega_n * Diffcoeff_w_g_s * (gradu_Pg_n * n_F_local))
                          + DCH4_w * (omega_s * Diffcoeff_w_w_s * (gradu_Pg_s * n_F_local) 
                          + omega_n * Diffcoeff_w_w_s * (gradu_Pg_n * n_F_local )) ;

      //double term_diffusion_w = -krW * (omega_s * rho_w_s * ((Kgradu_Pg_s * n_F_local) - (Kgradu_Pc_s * n_F_local))
      // + omega_n * rho_w_n * ((Kgradu_Pg_n * n_F_local) - (Kgradu_Pc_n * n_F_local)));
      ...
      for (size_type i = 0; i < lfsv_Pc_s.size(); i++)
      {
        r_s.accumulate(lfsv_Pc_s, i, term_nipg_w * krW * omega_s * rho_w_s * (Kn_F_s * gradpsi_Pc_s[i]) * factor);
      }
      for (size_type i = 0; i < lfsv_Pc_n.size(); i++)
      {
        r_n.accumulate(lfsv_Pc_n, i, term_nipg_w * krW * omega_n * rho_w_n * (Kn_F_n * gradpsi_Pc_n[i]) * factor);
      }
      // standard IP term integral
      for (size_type i = 0; i < lfsv_Pc_s.size(); i++)
      {
                      ...
                      R &r) const
  {
    ...
    for (const auto &ip : quadratureRule(geo, intorder))
    {
      ...
      if (bctype[Indices::PVId_Pg] == Indices::BCId_dirichlet)
      {
        ...
        double term_diffusion_g = -krN * (omega_s * rho_g_s * (Kgradu_Pg_s * n_F_local));
        double term_nipg_g = theta_g * (Pg_s - Pg_n);
        double term_penalty_g = penalty_factor_g * (Pg_s - Pg_n);
        // diffusion term
        for (size_type i = 0; i < lfsv_Pg_s.size(); i++)
        {
          r.accumulate(lfsv_Pg_s, i, term_diffusion_g * psi_Pg_s[i] * factor);
        }
        // (non-)symmetric IP term
        for (size_type i = 0; i < lfsv_Pg_s.size(); i++)
        {
          r.accumulate(lfsv_Pg_s, i, term_nipg_g * krN * omega_s * rho_g_s * (Kn_F_s * gradpsi_Pg_s[i]) * factor);
        }
        // standard IP term integral
        for (size_type i = 0; i < lfsv_Pg_s.size(); i++)
        {
      ...
      if (bctype[Indices::PVId_Pc] == Indices::BCId_dirichlet)
      {
        ...
        for (size_type i = 0; i < lfsv_Pc_s.size(); i++)
        {
          r.accumulate(lfsv_Pc_s, i, term_nipg_w * krW * omega_s * rho_w_s * (Kn_F_s * gradpsi_Pc_s[i]) * factor);
        }
        // standard IP term integral
        for (size_type i = 0; i < lfsv_Pc_s.size(); i++)
        {
