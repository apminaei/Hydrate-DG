...
{
private:
	IncludeProblemSpecifications includeSpecs;
	Hydrate hydrate;
	Methane methane;
	Water water;
	Soil soil;
...
public:
	...
	double diss_rateconstant(double T) const
	{
		...
		double k0 = includeSpecs.problemSpecifications.ReactionRateParam_k0();
		double deltaEbyR = includeSpecs.problemSpecifications.ReactionRateParam_deltaEabyR();
		double kd_diss = k0 * exp(-deltaEbyR / (T)); //defined in mol/m².Pa.s

		//		double kd_diss = includeSpecs.problemSpecs.ReactionRateParam_k0(); //used in kiel triax test
	...
	double form_rateconstant(double T) const
	{
		...
		double kd_form = includeSpecs.problemSpecifications.ReactionRateParam_kForm(); //defined in mol/m².Pa.s
		return kd_form;
	}
	...
	double sp_surfacearea(double Sh, double porosity, Dune::FieldVector<double, IncludeProblemSpecifications::ProblemSpecifications::dimension> globalPos) const
	{
		if (Sh < 1.e-6)
		{
			Sh = 0.;
		...
		double permeability = includeSpecs.problemSpecifications.SedimentPermeability(globalPos) * hydraulicProperties.KSF1(Sh) * hydraulicProperties.KSF2(porosity, includeSpecs.problemSpecifications.SedimentPorosity(globalPos));
	...
	double gasGenerationRate(double T, double Pg, double Sh, double Sw, double porosity,
							 Dune::FieldVector<double, IncludeProblemSpecifications::ProblemSpecifications::dimension> globalPos) const
	{
		double gas_gen = 0.0;
		...
		//												* ( (includeSpecs.problemSpecs.ProblemICValues(globalPos)[indices.PVId_Sh] - Sh)
		//														/includeSpecs.problemSpecs.ProblemICValues(globalPos)[indices.PVId_Sw]) );
		////		double Salinity = Salinity_0 * ( includeSpecs.problemSpecs.ProblemICValues(globalPos)[indices.PVId_Sw] / Sw ) ;
		//
		...
		{
			gas_gen = diss_rateconstant(T) * methane.molarMass() * sp_surfacearea(Sh, porosity, globalPos) * Tau_dissFactor(Sh, Sw, porosity) * includeSpecs.problemSpecifications.AreaFactor() * potential;
			/*			std::cout << "dissociation: " << 	diss_rateconstant( T )
										      * methane.molarMass()
										      * sp_surfacearea( Sh, porosity, globalPos )
										      * includeSpecs.problemSpecs.AreaFactor()
									      << std::endl;*/
		}
		else if (potential < potential_B)
		{
			gas_gen = form_rateconstant(T) * methane.molarMass() * sp_surfacearea(Sh, porosity, globalPos) * Tau_formFactor(Sh, Sw, porosity) * includeSpecs.problemSpecifications.AreaFactor() * potential;
		...
		else
		{
			...
			gas_gen = reactionRate * methane.molarMass() * sp_surfacearea(Sh, porosity, globalPos) * includeSpecs.problemSpecifications.AreaFactor() * potential;
		}
