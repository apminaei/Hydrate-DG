...
public :
	...
	value ( I& intersection,/*const typename GV::Traits::template Codim<0>::Entity& element,*/
		...
		if( property.mesh.isTopBoundary(globalPos)){
			...
			// 	Pw_top = icv[Indices::PVId_Pw] + (920. * 9.81 * press_rate * Xc_time * (time+dt)) /*should increase */
			// 									/ (property.characteristicValue.P_c);
			// 	T_top  = icv[Indices::PVId_T]+( property.parameter.DTz() * press_rate * Xc_time * (time+dt) )/ property.characteristicValue.T_c;
			// }
