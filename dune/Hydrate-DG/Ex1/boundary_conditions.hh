...
public:
		  ...
		  double dt /*s*/) const
	{
		...
		if (property.mesh.isTopBoundary(globalPos))
		{
			...
			bcvalue[indices.PVId_Sg] = Sg_top;
			 ...
			 double dt /*s*/) const
	{
		...
		if (property.mesh.isTopBoundary(globalPos))
		{
			...
			double Pw_top = icv[Indices::PVId_Pw] + (920. * 9.81 * press_rate * Xc_time * (time + dt)) / (property.characteristicValue.P_c);
			double xc_top = icv[Indices::PVId_C];
			double T_top = icv[Indices::PVId_T] + (0.035 * press_rate * Xc_time * (time + dt)) / property.characteristicValue.T_c;
