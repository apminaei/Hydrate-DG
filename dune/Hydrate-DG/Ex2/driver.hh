		   ...
		   )
{
	...
	double alpha_g = 1.e2 ;//* property.characteristicValue.X_source_mass;
	double alpha_w = 1.e2 ;//* property.characteristicValue.P_c;
	double alpha_s = 1.e2;
	double alpha_T = 1.e2 ;//* property.characteristicValue.T_c;
	double alpha_x = 1.e2 ;//* property.characteristicValue.X_source_mass;
	double alpha_y = 1.e2;
	double intorder=4;
	...
	*/
	...
	pdesolver.setVerbosityLevel(ptree.get("newton.verbosity",(int)3));
	pdesolver.setReduction(ptree.get("newton.reduction",(double)1e-5));
    pdesolver.setMinLinearReduction(ptree.get("newton.min_linear_reduction",(double)1.e-9));
	pdesolver.setAbsoluteLimit(ptree.get("newton.abs_error",(double)1.e-4)); 
	...
	while ( time < (t_END - 1e-3/Xc_t))
	{
		...
		try{
			...
			//newton_defects.resize(osm.getPDESolver().result().defects.size());
