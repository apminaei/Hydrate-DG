...
public:
	...
	double DynamicViscosity(double T, double Pg) const
	{
		...
		mu_0 *= (1. - (1. / (1.0707e-5)) * (4.8134e-14 * Pg - 4.1719e-21 * Pg * Pg + 7.3232e-28 * Pg * Pg * Pg)); // Pa.s -> ref: Frauenhofer Comsol Model
	...
	double Cp_res(double T, double Pg, double z_CH4) const
	{
		...
		double Cp_res = dda_T * (T / (2 * sqrt(2) * b)) * log((z_CH4 + (sqrt(2) + 1) * B) / (z_CH4 - (sqrt(2) - 1) * B)) + (Ru * pow(M - N, 2)) / (M * M - 2. * A * (z_CH4 + B)) - Ru;
