...
public:
	...
	// 			{
	// 				mat.accumulate(lfsv_Sg, i, lfsu_XCH4, j , por * rho_w * Sw * phi_XCH4[j] * psi_Pw[i] * factor);
	// 			}
	// 			for (size_type j = 0; j < lfsu_Sg.size(); j++)
	// 			{
	// 				mat.accumulate(lfsv_Sg, i, lfsu_Sg, j , por * (-rho_w * XCH4 + rho_g * (1. - YH2O)) * phi_Sg[j] * psi_Pw[i] * factor);
	// 			}
	// 			for (size_type j = 0; j < lfsu_Sh.size(); j++)
	// 			{
	// 				mat.accumulate(lfsv_Sg, i, lfsu_Sh, j , por * rho_w * XCH4 * -phi_Sh[j] * psi_Pw[i] * factor);
	// 			}
	// 			for (size_type j = 0; j < lfsu_YH2O.size(); j++)
	// 			{
	// 				mat.accumulate(lfsv_Sg, i, lfsu_YH2O, j , por * rho_g * Sg * -phi_YH2O[j] * psi_Pw[i] * factor);
	// 			}
	// 		}
	...
	// 			{
	// 				mat.accumulate(lfsv_XC, i, lfsu_XC, j , por * rho_w * Sw * phi_XC[j] * psi_Pw[i] * factor);
	// 			}
	// 			for (size_type j = 0; j < lfsu_Sg.size(); j++)
	// 			{
	// 				mat.accumulate(lfsv_XC, i, lfsu_Sg, j , por * rho_w * XC  * -phi_Sg[j] * psi_Pw[i] * factor);
	// 			}
	// 			for (size_type j = 0; j < lfsu_Sh.size(); j++)
	// 			{
	// 				mat.accumulate(lfsv_XC, i, lfsu_Sh, j , por * rho_w * XC * -phi_Sh[j] * psi_Pw[i] * factor);
	// 			}
	// 		}
	// 		for (size_type i = 0; i < lfsv_Pw.size(); i++)
	// 		{
