...
public:
	...
	void evaluate(){
		...
		for ( LeafIterator self = beginElem; self!= endElem; ++self )
		{
			...
			Dune::FieldVector<RF,dim> gradSg(0.);
			Dune::FieldVector<RF,dim> gradSh(0.);
	        evaluation_Pw->evalGradient(cell,cell_center_local,&gradP);
	        RF Pw/*ndim*/=0.;
	        evaluation_Pw->evalFunction(cell,cell_center_local,&Pw);
			evaluation_Sg->evalGradient(cell,cell_center_local,&gradSg);
	        RF Sg=0.;
			...
			evaluation_Sh->evalGradient(cell,cell_center_local,&gradSh);
	        RF Sh=0.;
...
// #endif
			auto K/*ndim*/ = param.soil.SedimentPermeabilityTensor( cell,cell_center_local );
			...
			K.mv(gravity, Kg);
			K.mv(gradP, Kgrad_Pw);
			K.mv(gradSg, Kgrad_sg);
			K.mv(gradSh, Kgrad_Sh);
			for(int d=0; d<dim; d++){
				ul_v[lfs_v.child(d).localIndex(0)] = - phase_flag *Xc_K * (lambda/Xc_mu) * ( Kgrad_Pw[d]*Xc_P/Xc_x + (rho*Xc_rho)*Kg[d] );
