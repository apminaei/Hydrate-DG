...
public:
  ...
  void alpha_volume(const EG &eg, const LFSU &lfsu_Sh, const X &x, const LFSV &lfsv_Sh, R &r) const
  {
    ...
    for (const auto &ip : quadratureRule(geo, intorder))
    {
      ...
      if (Pg < (P_eq+1.e-6) and Pg > (P_eq-1.e-6)){
        std::cout << ip_global << "  " << P_eq << std::endl;
      }
  ...
  //   std::vector<Dune::FieldVector<RF, dim>> Kgradphi_Pw(lfsu_Pw.size());
  //   std::vector<Dune::FieldVector<RF, dim>> Kgradphi_Sg(lfsu_Sg.size());
  //   std::vector<Dune::FieldVector<RF, dim>> Kgradphi_Sh(lfsu_Sh.size());
  ...
  //     // compute K * gradient of gradphi_Pw
  //     for (size_type i = 0; i < lfsu_Pw.size(); i++)
  //       K.mv(gradphi_Pw[i], Kgradphi_Pw[i]);


  //     // compute K * gradient of Sg
  //     K.mv(gradu_Sg, Kgradu_Sg);
  //     // compute K * gradient of gradphi_Sg
  //     for (size_type i = 0; i < lfsu_Sg.size(); i++)
  //       K.mv(gradphi_Sg[i], Kgradphi_Sg[i]);

  //     // compute K * gradient of Sh
  //     K.mv(gradu_Sh, Kgradu_Sh);
  //     // compute K * gradient of gradphi_Pw
  //     for (size_type i = 0; i < lfsu_Sh.size(); i++)
  //       K.mv(gradphi_Sh[i], Kgradphi_Sh[i]);
  ...
  //     //   {
  //     //     mat.accumulate(lfsv_Sg, i, lfsu_Sg, j, ( Xc_conv_m * rho_g * krN * (1. - YH2O) * -coeff_grad_Sw * Kgradphi_Sg[j]  ) * gradpsi_Sg[i] * factor);
  //     //   }
  //     //   for (size_type j = 0; j < lfsu_Sg.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Sg, i, lfsu_Sg, j, -Xc_diff_m * double(phi_Sg[j]) * (-rho_w * DCH4_w * gradu_XCH4 - rho_g * DH2O_g * gradu_YH2O ) * gradpsi_Sg[i] * factor);
  //     //   }
        

  //     //   for (size_type j = 0; j < lfsu_Sh.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Sg, i, lfsu_Sh, j, Xc_conv_m * rho_g * krN *(1. - YH2O) * (coeff_grad_Sh - coeff_grad_Sw) * Kgradphi_Sh[j] * gradpsi_Sg[i] * factor);
  //     //   }
  //     //   for (size_type j = 0; j < lfsu_Sh.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Sg, i, lfsu_Sh, j, -Xc_diff_m * (-rho_w * DCH4_w * gradu_XCH4) * double(phi_Sh[j]) * gradpsi_Sg[i] * factor);
  //     //   }

      
  //     //   for (size_type j = 0; j < lfsu_Pw.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Sg, i, lfsu_Pw, j, Xc_conv_m * (rho_g * krN *(1. - YH2O) + rho_w * krW * XCH4) * Kgradphi_Pw[j] * gradpsi_Sg[i] * factor);
  //     //   }

  //     //   for (size_type j = 0; j < lfsu_XCH4.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Sg, i, lfsu_XCH4, j, Xc_conv_m * rho_w * krW * (Kgradu_Pw - rho_w * Kg) * double(phi_XCH4[j]) * gradpsi_Sg[i] * factor);
  //     //   }
  //     //   for (size_type j = 0; j < lfsu_XCH4.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Sg, i, lfsu_XCH4, j, -Xc_diff_m * (rho_w * DCH4_w * Sw) * gradphi_XCH4[j] * gradpsi_Sg[i] * factor);
  //     //   }

  //     //   for (size_type j = 0; j < lfsu_YH2O.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Sg, i, lfsu_YH2O, j, Xc_conv_m * rho_g * krN * (Kgradu_Pg - rho_g * Kg) * -double(phi_YH2O[j]) * gradpsi_Sg[i] * factor);
  //     //   }
  //     //   for (size_type j = 0; j < lfsu_YH2O.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Sg, i, lfsu_YH2O, j, -Xc_diff_m * rho_g * DH2O_g * Sg * -gradphi_YH2O[j] * gradpsi_Sg[i] * factor);
  //     //   }
  //     // } // end of Methane component
  ...
  //     //   for (size_type j = 0; j < lfsu_Sg.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_XC, i, lfsu_Sg, j, -Xc_diff_m * (rho_w * DC_w * gradu_XC) * -double(phi_Sg[j]) * gradpsi_XC[i] * factor);
  //     //   }

  //     //   for (size_type j = 0; j < lfsu_Sh.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_XC, i, lfsu_Sh, j, -Xc_diff_m * (rho_w * DC_w * gradu_XC) * -double(phi_Sh[j]) * gradpsi_XC[i] * factor);
  //     //   }

  //     //   for (size_type j = 0; j < lfsu_Pw.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_XC, i, lfsu_Pw, j, Xc_conv_m * (rho_w * krW * XC) * Kgradphi_Pw[j] * gradpsi_XC[i] * factor);
  //     //   }

  //     //   for (size_type j = 0; j < lfsu_XC.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_XC, i, lfsu_XC, j, Xc_conv_m * rho_w * krW * (Kgradu_Pw - rho_w * Kg) * double(phi_XC[j]) * gradpsi_XC[i] * factor);
  //     //   }
  //     //   for (size_type j = 0; j < lfsu_XC.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_XC, i, lfsu_XC, j, -Xc_diff_m * (rho_w * DC_w * Sw) * gradphi_XC[j] * gradpsi_XC[i] * factor);
  //     //   }
  //     // } // end of Salt component
  ...
  //     //   for (size_type j = 0; j < lfsu_Sg.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Pw, i, lfsu_Sg, j, Xc_conv_m * rho_g * krN * YH2O * -coeff_grad_Sw * Kgradphi_Sg[j] * gradpsi_Pw[i] * factor);
  //     //   }
  //     //   for (size_type j = 0; j < lfsu_Sg.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Pw, i, lfsu_Sg, j, -Xc_diff_m * (rho_w * DCH4_w * gradu_XCH4 + rho_w * DC_w * gradu_XC + rho_g * DH2O_g * gradu_YH2O ) * double(phi_Sg[j]) * gradpsi_Pw[i] * factor);
  //     //   }

  //     //   for (size_type j = 0; j < lfsu_Sh.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Pw, i, lfsu_Sh, j, Xc_conv_m * rho_g * krN * YH2O * (coeff_grad_Sh - coeff_grad_Sw) * Kgradphi_Sh[j] * gradpsi_Pw[i] * factor);
  //     //   }
  //     //   for (size_type j = 0; j < lfsu_Sh.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Pw, i, lfsu_Sh, j, -Xc_diff_m * rho_w * (-DCH4_w * gradu_XCH4 -DC_w * gradu_XC) * -double(phi_Sh[j]) * gradpsi_Pw[i] * factor);
  //     //   }

  //     //   for (size_type j = 0; j < lfsu_Pw.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Pw, i, lfsu_Pw, j, Xc_conv_m * (rho_g * krN * YH2O + rho_w * krW * (1. - XC - XCH4)) * Kgradphi_Pw[j] * gradpsi_Pw[i] * factor);
  //     //   }

  //     //   for (size_type j = 0; j < lfsu_XCH4.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Pw, i, lfsu_XCH4, j, Xc_conv_m * rho_w * krW * (Kgradu_Pw - rho_w * Kg) * -double(phi_XCH4[j]) * gradpsi_Pw[i] * factor);
  //     //   }
  //     //   for (size_type j = 0; j < lfsu_XCH4.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Pw, i, lfsu_XCH4, j, -Xc_diff_m * (rho_w * DCH4_w * Sw) * -gradphi_XCH4[j] * gradpsi_Pw[i] * factor);
  //     //   }

  //     //   for (size_type j = 0; j < lfsu_YH2O.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Pw, i, lfsu_YH2O, j, Xc_conv_m * rho_g * krN * (Kgradu_Pg - rho_g * Kg) * double(phi_YH2O[j]) * gradpsi_Pw[i] * factor);
  //     //   }
  //     //   for (size_type j = 0; j < lfsu_YH2O.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Pw, i, lfsu_YH2O, j, -Xc_diff_m * rho_g * DH2O_g * Sg * gradphi_YH2O[j] * gradpsi_Pw[i] * factor);
  //     //   }

  //     //   for (size_type j = 0; j < lfsu_XC.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Pw, i, lfsu_XC, j, Xc_conv_m * rho_w * krW * (Kgradu_Pw - rho_w * Kg) * -double(phi_XC[j]) * gradpsi_Pw[i] * factor);
  //     //   }
  //     //   for (size_type j = 0; j < lfsu_XC.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_Pw, i, lfsu_XC, j, -Xc_diff_m * (rho_w * DC_w * Sw) * -gradphi_XC[j] * gradpsi_Pw[i] * factor);
  //     //   }

  //     // } // end of Water component
  ...
  //     //     tmp = 1.;
	// 		// 	for (size_type j = 0; j < lfsu_Sg.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_YH2O, i, lfsu_Sg, j, (1. - tmp) * phi_Sg[j] * psi_YH2O[i] * factor);
  //     //   }
        
  //     //   for (size_type j = 0; j < lfsu_YH2O.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_YH2O, i, lfsu_YH2O, j, (-tmp) * phi_YH2O[j] * psi_YH2O[i] * factor);
  //     //   }
  ...
  //     //     tmp = 1.;
	// 		// 	for (size_type j = 0; j < lfsu_Sg.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_XCH4, i, lfsu_Sg, j, (1. - tmp) * -phi_Sg[j] * psi_XCH4[i] * factor);
  //     //   }
  //     //   for (size_type j = 0; j < lfsu_Sh.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_XCH4, i, lfsu_Sh, j, (1. - tmp) * -phi_Sh[j] * psi_XCH4[i] * factor);
  //     //   }
  //     //   for (size_type j = 0; j < lfsu_XCH4.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_XCH4, i, lfsu_XCH4, j, (-tmp) * phi_XCH4[j] * psi_XCH4[i] * factor);
  //     //   }
  //     //   for (size_type j = 0; j < lfsu_XC.size(); j++)
  //     //   {
  //     //     mat.accumulate(lfsv_XCH4, i, lfsu_XC, j, (-tmp) * -phi_XC[j] * psi_XCH4[i] * factor);
  //     //   }
	// 		// } // end of NCP2
  ...
  //       // for (size_type j = 0; j < lfsu_Sg.size(); j++)
  //       // {
  //       //   mat.accumulate(lfsv_T, i, lfsu_Sg, j, Xc_conv_h * rho_g * krN * Cp_g * (T - T_ref) * -coeff_grad_Sw * Kgradphi_Sg[j] * gradpsi_T[i] * factor);
  //       // }

  //       // for (size_type j = 0; j < lfsu_Sh.size(); j++)
  //       // {
  //       //   mat.accumulate(lfsv_T, i, lfsu_Sh, j, Xc_conv_h * rho_g * krN * Cp_g * (T - T_ref) * (coeff_grad_Sh - coeff_grad_Sw) * Kgradphi_Sh[j] * gradpsi_T[i] * factor);
  //       // }

  //       // for (size_type j = 0; j < lfsu_Pw.size(); j++)
  //       // {
  //       //   mat.accumulate(lfsv_T, i, lfsu_Pw, j, Xc_conv_h * (rho_g * Cp_g * krN + rho_w * Cp_w * krW) * (T - T_ref) * Kgradphi_Pw[j] * gradpsi_T[i] * factor);
  //       // }
  ...
  // //   std::vector<RF> Phi_Pw_s(lfsu_Pw_s.size());
    
  // //   std::vector<RF> Phi_Sg_s(lfsu_Sg_s.size());
    
  // //   std::vector<RF> Phi_Sh_s(lfsu_Sh_s.size());
    
  // //   std::vector<RF> Phi_T_s(lfsu_T_s.size());
   
  // //   std::vector<RF> Phi_XCH4_s(lfsu_XCH4_s.size());
    
  // //   std::vector<RF> Phi_YH2O_s(lfsu_YH2O_s.size());
    
  // //   std::vector<RF> Phi_XC_s(lfsu_XC_s.size());
    

  // //   std::vector<RF> Phi_Pw_n(lfsu_Pw_n.size());
    
  // //   std::vector<RF> Phi_Sg_n(lfsu_Sg_n.size());
    
  // //   std::vector<RF> Phi_Sh_n(lfsu_Sh_n.size());
    
  // //   std::vector<RF> Phi_T_n(lfsu_T_n.size());
    
  // //   std::vector<RF> Phi_XCH4_n(lfsu_XCH4_n.size());
    
  // //   std::vector<RF> Phi_YH2O_n(lfsu_YH2O_n.size());
    
  // //   std::vector<RF> Phi_XC_n(lfsu_XC_n.size());
    

  // //   std::vector<Dune::FieldVector<RF, dim>> Kgradphi_Pw_s(lfsu_Pw_s.size());
  // //   std::vector<Dune::FieldVector<RF, dim>> Kgradphi_Sg_s(lfsu_Sg_s.size());
  // //   std::vector<Dune::FieldVector<RF, dim>> Kgradphi_Sh_s(lfsu_Sh_s.size());

  // //   std::vector<Dune::FieldVector<RF, dim>> Kgradphi_Pw_n(lfsu_Pw_n.size());
  // //   std::vector<Dune::FieldVector<RF, dim>> Kgradphi_Sg_n(lfsu_Sg_n.size());
  // //   std::vector<Dune::FieldVector<RF, dim>> Kgradphi_Sh_n(lfsu_Sh_n.size());
  ...
  // //     for (size_type i = 0; i < lfsu_Pw_s.size(); i++){
  // //       Pw_s += x_s(lfsu_Pw_s, i) * phi_Pw_s[i];
  // //       Phi_Pw_s[i] = phi_Pw_s[i]; 
  // //     }
  // //     RF Pw_n = 0.0;
  // //     for (size_type i = 0; i < lfsu_Pw_n.size(); i++){
  // //       Pw_n += x_n(lfsu_Pw_n, i) * phi_Pw_n[i];
  // //       Phi_Pw_n[i] = phi_Pw_n[i]; 
  // //     }
  ...
  // //     for (size_type i = 0; i < lfsu_Sg_s.size(); i++){
  // //       Sg_s += x_s(lfsu_Sg_s, i) * phi_Sg_s[i];
  // //       Phi_Sg_s[i] = phi_Sg_s[i]; 
  // //     }
  // //     RF Sg_n = 0.0;
  // //     for (size_type i = 0; i < lfsu_Sg_n.size(); i++){
  // //       Sg_n += x_n(lfsu_Sg_n, i) * phi_Sg_n[i];
  // //       Phi_Sg_n[i] = phi_Sg_n[i]; 
  // //     }
  ...
  // //     for (size_type i = 0; i < lfsu_Sh_s.size(); i++){
  // //       Sh_s += x_s(lfsu_Sh_s, i) * phi_Sh_s[i];
  // //       Phi_Sh_s[i] = phi_Sh_s[i]; 
  // //     }
  // //     RF Sh_n = 0.0;
  // //     for (size_type i = 0; i < lfsu_Sh_n.size(); i++){
  // //       Sh_n += x_n(lfsu_Sh_n, i) * phi_Sh_n[i];
  // //       Phi_Sh_n[i] = phi_Sh_n[i]; 
  // //     }
  ...
  // //     for (size_type i = 0; i < lfsu_XCH4_s.size(); i++){
  // //       XCH4_s += x_s(lfsu_XCH4_s, i) * phi_XCH4_s[i];
  // //       Phi_XCH4_s[i] = phi_XCH4_s[i]; 
  // //     }
  // //     RF XCH4_n = 0.0;
  // //     for (size_type i = 0; i < lfsu_XCH4_n.size(); i++){
  // //       XCH4_n += x_n(lfsu_XCH4_n, i) * phi_XCH4_n[i];
  // //       Phi_XCH4_n[i] = phi_XCH4_n[i]; 
  // //     }
  ...
  // //     for (size_type i = 0; i < lfsu_YH2O_s.size(); i++){
  // //       YH2O_s += x_s(lfsu_YH2O_s, i) * phi_YH2O_s[i];
  // //       Phi_YH2O_s[i] = phi_YH2O_s[i]; 
  // //     }
  // //     RF YH2O_n = 0.0;
  // //     for (size_type i = 0; i < lfsu_YH2O_n.size(); i++){
  // //       YH2O_n += x_n(lfsu_YH2O_n, i) * phi_YH2O_n[i];
  // //       Phi_YH2O_n[i] = phi_YH2O_n[i]; 
  // //     }
  ...
  // //     for (size_type i = 0; i < lfsu_XC_s.size(); i++){
  // //       XC_s += x_s(lfsu_XC_s, i) * phi_XC_s[i];
  // //       Phi_XC_s[i] = phi_XC_s[i]; 
  // //     }
  // //     RF XC_n = 0.0;
  // //     for (size_type i = 0; i < lfsu_XC_n.size(); i++){
  // //       XC_n += x_n(lfsu_XC_n, i) * phi_XC_n[i];
  // //       Phi_XC_n[i] = phi_XC_n[i]; 
  // //     }
  ...
  // //     K_s.mv(gradu_Pw_s, Kgradu_Pw_s);
  // //     K_n.mv(gradu_Pw_n, Kgradu_Pw_n);
  // //     K_s.mv(gradphi_Pw_s, Kgradphi_Pw_s);
  // //     K_n.mv(gradphi_Pw_n, Kgradphi_Pw_n);

  // //     // compute K * gradient of Sg
  // //     K_s.mv(gradu_Sg_s, Kgradu_Sg_s);
  // //     K_n.mv(gradu_Sg_n, Kgradu_Sg_n);
  // //     K_s.mv(gradphi_Sg_s, Kgradphi_Sg_s);
  // //     K_n.mv(gradphi_Sg_n, Kgradphi_Sg_n);

  // //     // compute K * gradient of Sh
  // //     K_s.mv(gradu_Sh_s, Kgradu_Sh_s);
  // //     K_n.mv(gradu_Sh_n, Kgradu_Sh_n);
  // //     K_s.mv(gradphi_Sh_s, Kgradphi_Sh_s);
  // //     K_n.mv(gradphi_Sh_n, Kgradphi_Sh_n);
  ...
  // //     // ===================================================================== Methane ==============================================================  
  // //     // Methane component-wise mass-balance
  // //     // derivative of Methane comp. wrt Sg
  // //     for (size_type j = 0; j < lfsu_Sg_s.size(); j++)
  // //     {
  // //       tmp =  - Xc_conv_m * omegaup_g_s * rho_g_s * krN_s * (1. - YH2O_s) * (-coeff_grad_Sw_s) * Kgradphi_Sg_s[j] * n_F_local;
  // //       tmp +=  0.5 * Xc_diff_m * (rho_w_s * DCH4_w_s * gradu_XCH4_s + rho_g_s * DH2O_g_s * gradu_YH2O_s) * (-Phi_Sg_s[j]) * n_F_local; 
  // //       tmp += penalty_factor_s * Phi_Sg_s[j];
  // //       auto term_nipg_s = -Xc_conv_m * theta_g * krN_s * omegaup_g_s * rho_g_s * (1. -  YH2O_s) * (- coeff_grad_Sw_s) * Kn_F_s * (Phi_Sg_s[j]);
  // //       auto term_nipg_n = -Xc_conv_m * theta_g * krN_n * omegaup_g_n * rho_g_n * (1. -  YH2O_n) * (- coeff_grad_Sw_n) * Kn_F_n * (Phi_Sg_s[j]);

  // //       for (size_type i = 0; i < lfsv_Sg_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_Sg_s, i, lfsu_Sg_s, j, (tmp * psi_Sg_s[i] + term_nipg_s * gradpsi_Sg_s[i] + term_nipg_n * gradpsi_Sg_n[i] ) * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Sg_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_Sg_n, i, lfsu_Sg_s, j, tmp * -psi_Sg_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_Sg_n.size(); j++)
  // //     {
  // //       tmp =  - Xc_conv_m * omegaup_g_n * rho_g_n * krN_n * (1. - YH2O_n) * (-coeff_grad_Sw_n) * Kgradphi_Sg_n[j] * n_F_local; 
  // //       tmp +=  0.5 * Xc_diff_m * (rho_w_n * DCH4_w_n * gradu_XCH4_n + rho_g_n * DH2O_g_n * gradu_YH2O_n) * (-(Phi_Sg_n[j])) * n_F_local;
  // //       tmp += penalty_factor_s * -Phi_Sg_n[j];
  // //       auto term_nipg_s = -Xc_conv_m * theta_g * krN_s * omegaup_g_s * rho_g_s * (1. -  YH2O_s) * (- coeff_grad_Sw_s) * Kn_F_s * (-Phi_Sg_n[j]);
  // //       auto term_nipg_n = -Xc_conv_m * theta_g * krN_n * omegaup_g_n * rho_g_n * (1. -  YH2O_n) * (- coeff_grad_Sw_n) * Kn_F_n * (-Phi_Sg_n[j]);
  // //       for (size_type i = 0; i < lfsv_Sg_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_Sg_s, i, lfsu_Sg_n, j, tmp * psi_Sg_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Sg_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_Sg_n, i, lfsu_Sg_n, j, (tmp * -psi_Sg_n[i] + term_nipg_s * gradpsi_Sg_s[i] + term_nipg_n * gradpsi_Sg_n[i] ) * factor );
  // //       }
  // //     }tmp=0.;
  // //     // derivative of Methane comp. wrt Sh
  // //     for (size_type j = 0; j < lfsu_Sh_s.size(); j++)
  // //     {
  // //         tmp =  - Xc_conv_m * omegaup_g_s * rho_g_s * krN_s * (1. - YH2O_s) * (coeff_grad_Sh_s - coeff_grad_Sw_s) * Kgradphi_Sh_s[j] * n_F_local; 
  // //         tmp +=  0.5 * Xc_diff_m * (rho_w_s * DCH4_w_s * gradu_XCH4_s ) * (-double(phi_Sh_s[j])) * n_F_local;
  // //       for (size_type i = 0; i < lfsv_Sg_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_Sg_s, i, lfsu_Sh_s, j, tmp * psi_Sg_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Sg_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_Sg_n, i, lfsu_Sh_s, j, tmp * -psi_Sg_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_Sh_n.size(); j++)
  // //     {
  // //        tmp =  - Xc_conv_m * omegaup_g_n * rho_g_n * krN_n * (1. - YH2O_n) * (coeff_grad_Sh_n - coeff_grad_Sw_n) * Kgradphi_Sh_n[j] * n_F_local; 
  // //        tmp +=  0.5 * Xc_diff_m * (rho_w_n * DCH4_w_n * gradu_XCH4_n ) * (-double(phi_Sh_n[j])) * n_F_local;
  // //       for (size_type i = 0; i < lfsv_Sg_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_Sg_s, i, lfsu_Sh_n, j, tmp * psi_Sg_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Sg_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_Sg_n, i, lfsu_Sh_n, j, tmp * -psi_Sg_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     // derivative of Methane comp. wrt YH2O
  // //     for (size_type j = 0; j < lfsu_YH2O_s.size(); j++)
  // //     {
  // //       tmp =  - Xc_conv_m * omegaup_g_s * rho_g_s * krN_s * ( -double(phi_YH2O_s[j])) * (Kgradu_Pg_s - rho_g_s * Kg_s) * n_F_local;
  // //       tmp +=  0.5 * Xc_diff_m * ( rho_g_s * DH2O_g_s * Sg_s) * (-gradphi_YH2O_s[j]) * n_F_local;
  // //       auto term_nipg_s = -Xc_conv_m * theta_g * krN_s * omegaup_g_s * rho_g_s * (- coeff_grad_Sw_s) * Kn_F_s * double(-phi_YH2O_s[j]);
         
  // //       for (size_type i = 0; i < lfsv_Sg_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_Sg_s, i, lfsu_YH2O_s, j, (tmp * psi_Sg_s[i] + term_nipg_s * gradpsi_Sg_s[i] * Sg_s)  * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Sg_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_Sg_n, i, lfsu_YH2O_s, j, (tmp * -psi_Sg_n[i] + term_nipg_s * gradpsi_Sg_s[i] * -Sg_n) * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_YH2O_n.size(); j++)
  // //     {
  // //       tmp =  - Xc_conv_m * omegaup_g_n * rho_g_n * krN_n * ( -double(phi_YH2O_n[j])) * (Kgradu_Pg_n - rho_g_n * Kg_n)  * n_F_local;
  // //       tmp +=  0.5 * Xc_diff_m * ( rho_g_n * DH2O_g_n * Sg_n) * (-gradphi_YH2O_n[j]) * n_F_local;
  // //       auto term_nipg_n = -Xc_conv_m * theta_g * krN_n * omegaup_g_n * rho_g_n * (- coeff_grad_Sw_n) * Kn_F_n * double(-phi_YH2O_n[j]); 
  // //       for (size_type i = 0; i < lfsv_Sg_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_Sg_s, i, lfsu_YH2O_n, j, (tmp * psi_Sg_s[i] + term_nipg_n * gradpsi_Sg_n[i] * Sg_s) * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Sg_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_Sg_n, i, lfsu_YH2O_n, j, (tmp * -psi_Sg_n[i] + term_nipg_n * gradpsi_Sg_n[i] * -Sg_n) * factor );
  // //       }
  // //     }tmp=0.;
  // //     // derivative of Methane comp. wrt XCH4
  // //     for (size_type j = 0; j < lfsu_XCH4_s.size(); j++)
  // //     {
  // //         tmp =  - Xc_conv_m * omegaup_w_s * rho_w_s * krW_s * ( double(phi_XCH4_s[j])) * (Kgradu_Pw_s - rho_w_s * Kg_s) * n_F_local;
  // //         tmp +=  0.5 * Xc_diff_m * (rho_w_s * DCH4_w_s * Sw_s ) * (gradphi_XCH4_s[j]) * n_F_local; 
  // //       for (size_type i = 0; i < lfsv_Sg_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_Sg_s, i, lfsu_XCH4_s, j, tmp * psi_Sg_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Sg_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_Sg_n, i, lfsu_XCH4_s, j, tmp * -psi_Sg_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_XCH4_n.size(); j++)
  // //     {
  // //        tmp =  - Xc_conv_m * omegaup_w_n * rho_w_n * krW_n * ( double(phi_XCH4_n[j])) * (Kgradu_Pw_n - rho_w_n * Kg_n)  * n_F_local; 
  // //        tmp +=  0.5 * Xc_diff_m * (rho_w_n * DCH4_w_n * Sw_n ) * (gradphi_XCH4_n[j]) * n_F_local;
  // //       for (size_type i = 0; i < lfsv_Sg_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_Sg_s, i, lfsu_XCH4_n, j, tmp * psi_Sg_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Sg_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_Sg_n, i, lfsu_XCH4_n, j, tmp * -psi_Sg_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     // derivative of Methane comp. wrt Pw
  // //     for (size_type j = 0; j < lfsu_Pw_s.size(); j++)
  // //     {
  // //         tmp =  - Xc_conv_m * (omegaup_w_s * rho_w_s * krW_s * XCH4_s + omegaup_g_s * rho_g_s * krN_s * (1. - YH2O_s))* Kgradphi_Pw_s[j] * n_F_local; 
  // //       for (size_type i = 0; i < lfsv_Sg_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_Sg_s, i, lfsu_Pw_s, j, tmp * psi_Sg_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Sg_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_Sg_n, i, lfsu_Pw_s, j, tmp * -psi_Sg_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_Pw_n.size(); j++)
  // //     {
  // //        tmp =  - Xc_conv_m * (omegaup_w_n * rho_w_n * krW_n * XCH4_n + omegaup_g_n * rho_g_n * krN_n * (1. - YH2O_n))* Kgradphi_Pw_n[j] * n_F_local; 
  // //       for (size_type i = 0; i < lfsv_Sg_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_Sg_s, i, lfsu_Pw_n, j, tmp * psi_Sg_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Sg_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_Sg_n, i, lfsu_Pw_n, j, tmp * -psi_Sg_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //   // ===================================================================== End Methane ========================================================
  // //   // ===================================================================== Water ==============================================================
  // //     // Water component-wise mass-balance
  // //     // derivative of Water comp. wrt Sg
  // //     for (size_type j = 0; j < lfsu_Sg_s.size(); j++)
  // //     {
  // //       tmp =  - Xc_conv_m * omegaup_g_s * rho_g_s * krN_s *  YH2O_s * (-coeff_grad_Sw_s) * Kgradphi_Sg_s[j] * n_F_local;
  // //       tmp +=  0.5 * Xc_diff_m * (rho_w_s * (DCH4_w_s * gradu_XCH4_s + DC_w_s * gradu_XC_s) + rho_g_s * DH2O_g_s * gradu_YH2O_s) * (double(phi_Sg_s[j])) * n_F_local;
          
  // //       for (size_type i = 0; i < lfsv_Pw_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_Pw_s, i, lfsu_Sg_s, j, (tmp * psi_Pw_s[i] )   * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Pw_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_Pw_n, i, lfsu_Sg_s, j, tmp * -psi_Pw_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_Sg_n.size(); j++)
  // //     {
  // //       tmp =  - Xc_conv_m * omegaup_g_n * rho_g_n * krN_n *  YH2O_n * (-coeff_grad_Sw_n) * Kgradphi_Sg_n[j]* n_F_local;
  // //       tmp +=  0.5 * Xc_diff_m * (rho_w_n * (DCH4_w_n * gradu_XCH4_n + DC_w_n * gradu_XC_n) + rho_g_n * DH2O_g_n * gradu_YH2O_n) * (double(phi_Sg_n[j])) * n_F_local;
          
  // //       for (size_type i = 0; i < lfsv_Pw_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_Pw_s, i, lfsu_Sg_n, j, tmp * psi_Pw_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Pw_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_Pw_n, i, lfsu_Sg_n, j, tmp * -psi_Pw_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     // derivative of Water comp. wrt Sh
  // //     for (size_type j = 0; j < lfsu_Sh_s.size(); j++)
  // //     {
  // //         tmp =  - Xc_conv_m * omegaup_g_s * rho_g_s * krN_s * YH2O_s * (coeff_grad_Sh_s - coeff_grad_Sw_s) * Kgradphi_Sh_s[j] * n_F_local; 
  // //         tmp +=  0.5 * Xc_diff_m * (rho_w_s * (DCH4_w_s * gradu_XCH4_s + DC_w_s * gradu_XC_s) ) * (double(phi_Sh_s[j])) * n_F_local;
  // //       for (size_type i = 0; i < lfsv_Pw_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_Pw_s, i, lfsu_Sh_s, j, tmp * psi_Pw_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Pw_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_Pw_n, i, lfsu_Sh_s, j, tmp * -psi_Pw_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_Sh_n.size(); j++)
  // //     {
  // //        tmp =  - Xc_conv_m * omegaup_g_n * rho_g_n * krN_n * YH2O_n * (coeff_grad_Sh_n - coeff_grad_Sw_n) * Kgradphi_Sh_n[j] * n_F_local; 
  // //        tmp +=  0.5 * Xc_diff_m * (rho_w_n *(DCH4_w_n * gradu_XCH4_n + DC_w_n * gradu_XC_n) ) * (double(phi_Sh_n[j])) * n_F_local;
  // //       for (size_type i = 0; i < lfsv_Pw_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_Pw_s, i, lfsu_Sh_n, j, tmp * psi_Pw_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Pw_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_Pw_n, i, lfsu_Sh_n, j, tmp * -psi_Pw_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     // derivative of Water comp. wrt YH2O
  // //     for (size_type j = 0; j < lfsu_YH2O_s.size(); j++)
  // //     {
  // //         tmp =  - Xc_conv_m * omegaup_g_s * rho_g_s * krN_s * ( double(phi_YH2O_s[j])) * (Kgradu_Pg_s - rho_g_s * Kg_s) * n_F_local;
  // //         tmp +=  0.5 * Xc_diff_m * ( rho_g_s * DH2O_g_s * Sg_s) * (gradphi_YH2O_s[j]) * n_F_local;
  // //       auto term_nipg_s = -Xc_conv_m * theta_w * krN_s * omegaup_g_s * rho_g_s * (double(phi_YH2O_s[j])) * Kn_F_s ;  
  // //       for (size_type i = 0; i < lfsv_Pw_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_Pw_s, i, lfsu_YH2O_s, j, (tmp * psi_Pw_s[i] + term_nipg_s * gradpsi_Pw_s[i] * Pw_s) * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Pw_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_Pw_n, i, lfsu_YH2O_s, j, (tmp * -psi_Pw_n[i] + term_nipg_s * gradpsi_Pw_s[i] * -Pw_n) * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_YH2O_n.size(); j++)
  // //     {
  // //        tmp =  - Xc_conv_m * omegaup_g_n * rho_g_n * krN_n * ( double(phi_YH2O_n[j])) * (Kgradu_Pg_n - rho_g_n * Kg_n)  * n_F_local;
  // //        tmp +=  0.5 * Xc_diff_m * ( rho_g_n * DH2O_g_n * Sg_n) * (gradphi_YH2O_n[j]) * n_F_local;
  // //       auto term_nipg_n = -Xc_conv_m * theta_w * krN_n * omegaup_g_n * rho_g_n * (double(phi_YH2O_n[j])) * Kn_F_n ;  
  // //       for (size_type i = 0; i < lfsv_Pw_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_Pw_s, i, lfsu_YH2O_n, j, (tmp * psi_Pw_s[i] + term_nipg_n * gradpsi_Pw_n[i] * Pw_s) * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Pw_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_Pw_n, i, lfsu_YH2O_n, j, (tmp * -psi_Pw_n[i] + term_nipg_n * gradpsi_Pw_n[i] * -Pw_n) * factor );
  // //       }
  // //     }tmp=0.;
  // //     // derivative of Water comp. wrt XCH4
  // //     for (size_type j = 0; j < lfsu_XCH4_s.size(); j++)
  // //     {
  // //         tmp =  - Xc_conv_m * omegaup_w_s * rho_w_s * krW_s * ( -double(phi_XCH4_s[j])) * (Kgradu_Pw_s - rho_w_s * Kg_s) * n_F_local;
  // //         tmp +=  0.5 * Xc_diff_m * (rho_w_s * DCH4_w_s * Sw_s ) * (-gradphi_XCH4_s[j]) * n_F_local;
  // //       auto term_nipg_s = -Xc_conv_m * theta_w * krW_s * omegaup_w_s * rho_w_s * (-double(phi_XCH4_s[j])) * Kn_F_s ; 
  // //       for (size_type i = 0; i < lfsv_Pw_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_Pw_s, i, lfsu_XCH4_s, j, (tmp * psi_Pw_s[i] + term_nipg_s * gradpsi_Pw_s[i] * Pw_s) * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Pw_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_Pw_n, i, lfsu_XCH4_s, j, (tmp * -psi_Pw_n[i] + term_nipg_s * gradpsi_Pw_s[i] * -Pw_n) * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_XCH4_n.size(); j++)
  // //     {
  // //        tmp =  - Xc_conv_m * omegaup_w_n * rho_w_n * krW_n * ( -double(phi_XCH4_n[j])) * (Kgradu_Pw_n - rho_w_n * Kg_n)  * n_F_local; 
  // //        tmp +=  0.5 * Xc_diff_m * (rho_w_n * DCH4_w_n * Sw_n ) * (-gradphi_XCH4_n[j]) * n_F_local;
  // //       auto term_nipg_n = -Xc_conv_m * theta_w * krW_n * omegaup_w_n * rho_w_n * -double(phi_XCH4_n[j]) * Kn_F_n ;
  // //       for (size_type i = 0; i < lfsv_Pw_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_Pw_s, i, lfsu_XCH4_n, j, (tmp * psi_Pw_s[i] + term_nipg_n * gradpsi_Pw_n[i] * Pw_s) * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Pw_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_Pw_n, i, lfsu_XCH4_n, j, (tmp * -psi_Pw_n[i] + term_nipg_n * gradpsi_Pw_n[i] * -Pw_n) * factor );
  // //       }
  // //     }tmp=0.;
  // //     // derivative of Water comp. wrt XC
  // //     for (size_type j = 0; j < lfsu_XC_s.size(); j++)
  // //     {
  // //       tmp =  - Xc_conv_m * omegaup_w_s * rho_w_s * krW_s * ( -double(phi_XC_s[j])) * (Kgradu_Pw_s - rho_w_s * Kg_s) * n_F_local;
  // //       tmp +=  0.5 * Xc_diff_m * (rho_w_s * DC_w_s * Sw_s ) * (-gradphi_XC_s[j]) * n_F_local;
  // //       auto term_nipg_s = -Xc_conv_m * theta_w * krW_s * omegaup_w_s * rho_w_s * (-double(phi_XC_s[j])) * Kn_F_s ;
         
  // //       for (size_type i = 0; i < lfsv_Pw_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_Pw_s, i, lfsu_XC_s, j, (tmp * psi_Pw_s[i] + term_nipg_s * gradpsi_Pw_s[i] * Pw_s) * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Pw_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_Pw_n, i, lfsu_XC_s, j, (tmp * -psi_Pw_n[i] + term_nipg_s * gradpsi_Pw_s[i] * -Pw_n) * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_XC_n.size(); j++)
  // //     {
  // //       tmp =  - Xc_conv_m * omegaup_w_n * rho_w_n * krW_n * ( -double(phi_XC_n[j])) * (Kgradu_Pw_n - rho_w_n * Kg_n)  * n_F_local; 
  // //       tmp +=  0.5 * Xc_diff_m * (rho_w_n * DC_w_n * Sw_n ) * (-gradphi_XC_n[j]) * n_F_local;
  // //       auto term_nipg_n = -Xc_conv_m * theta_w * krW_n * omegaup_w_n * rho_w_n * -double(phi_XC_n[j]) * Kn_F_n ;
  // //       for (size_type i = 0; i < lfsv_Pw_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_Pw_s, i, lfsu_XC_n, j, (tmp * psi_Pw_s[i] + term_nipg_n * gradpsi_Pw_n[i] * Pw_s) * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Pw_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_Pw_n, i, lfsu_XC_n, j, (tmp * -psi_Pw_n[i] + term_nipg_n * gradpsi_Pw_n[i] * -Pw_n) * factor );
  // //       }
  // //     }tmp=0.;
  // //     // derivative of Water comp. wrt Pw
  // //     for (size_type j = 0; j < lfsu_Pw_s.size(); j++)
  // //     {
  // //       tmp = -Xc_conv_m * (omegaup_w_s * rho_w_s * krW_s * (1. -XC_s -XCH4_s) + omegaup_g_s * rho_g_s * krN_s *  YH2O_s)* Kgradphi_Pw_s[j] * n_F_local; 
  // //       tmp += penalty_factor_w * phi_Pw_s[j]; // Penalty term
  // //       auto term_nipg_s = -Xc_conv_m * theta_w * (krW_s * omegaup_w_s * rho_w_s * (1. - XC_s - XCH4_s) + krN_s * omegaup_g_s * rho_g_s * YH2O_s)* Kn_F_s * double(phi_Pw_s[j]);
  // //       auto term_nipg_n = -Xc_conv_m * theta_w * (krW_n * omegaup_w_n * rho_w_n * (1. - XC_n - XCH4_n) + krN_n * omegaup_g_n * rho_g_n * YH2O_n)* Kn_F_n * double(phi_Pw_s[j]);
  // //       for (size_type i = 0; i < lfsv_Pw_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_Pw_s, i, lfsu_Pw_s, j, (tmp * psi_Pw_s[i] + term_nipg_s * gradpsi_Pw_s[i] + term_nipg_n * gradpsi_Pw_n[i]) * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Pw_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_Pw_n, i, lfsu_Pw_s, j, tmp * -psi_Pw_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_Pw_n.size(); j++)
  // //     {
  // //       tmp =  - Xc_conv_m * (omegaup_w_n * rho_w_n * krW_n * (1. -XC_n -XCH4_n) + omegaup_g_n * rho_g_n * krN_n *  YH2O_n)* Kgradphi_Pw_n[j] * n_F_local;
  // //       tmp += penalty_factor_w * -phi_Pw_n[j]; // Penalty term
  // //       auto term_nipg_s = -Xc_conv_m * theta_w * (krW_s * omegaup_w_s * rho_w_s * (1. - XC_s - XCH4_s) + krN_s * omegaup_g_s * rho_g_s * YH2O_s)* Kn_F_s * double(-phi_Pw_n[j]);
  // //       auto term_nipg_n = -Xc_conv_m * theta_w * (krW_n * omegaup_w_n * rho_w_n * (1. - XC_n - XCH4_n) + krN_n * omegaup_g_n * rho_g_n * YH2O_n)* Kn_F_n * double(-phi_Pw_n[j]); 
  // //       for (size_type i = 0; i < lfsv_Pw_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_Pw_s, i, lfsu_Pw_n, j, tmp * psi_Pw_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Pw_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_Pw_n, i, lfsu_Pw_n, j, (tmp * -psi_Pw_n[i]  + term_nipg_s * gradpsi_Pw_s[i] + term_nipg_n * gradpsi_Pw_n[i]) * factor );
  // //       }
  // //     }tmp=0.;
  // //   // =================================================================== End Water ============================================================
  ...
  // //   // ===================================================================== Salt ==============================================================
  // //     // Salt component-wise mass-balance
  // //     // derivative of Salt comp. wrt Sg
  // //     for (size_type j = 0; j < lfsu_Sg_s.size(); j++)
  // //     {
  // //       tmp =  0.5 * Xc_diff_m * rho_w_s * DC_w_s * gradu_XC_s  * (-double(phi_Sg_s[j])) * n_F_local; 
  // //       auto term_nipg_s = 0.5 * Xc_diff_m * theta_x * rho_w_s * DC_w_s * (-double(phi_Sg_s[j])) * n_F_local;
  // //       for (size_type i = 0; i < lfsv_XC_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_XC_s, i, lfsu_Sg_s, j, (tmp * psi_XC_s[i] + term_nipg_s * gradpsi_XC_s[i] * XC_s) * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_XC_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_XC_n, i, lfsu_Sg_s, j, (tmp * -psi_XC_n[i] + term_nipg_s * gradpsi_XC_s[i] * -XC_n) * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_Sg_n.size(); j++)
  // //     {
  // //       tmp =  0.5 * Xc_diff_m * rho_w_n * DC_w_n * gradu_XC_n * (-double(phi_Sg_n[j])) * n_F_local;
  // //       auto term_nipg_n = 0.5 * Xc_diff_m * theta_x * rho_w_n * DC_w_n * (-double(phi_Sg_n[j])) * n_F_local;
  // //       for (size_type i = 0; i < lfsv_XC_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_XC_s, i, lfsu_Sg_n, j, (tmp * psi_XC_s[i] + term_nipg_n * gradpsi_XC_n[i] * XC_s) * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_XC_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_XC_n, i, lfsu_Sg_n, j, (tmp * -psi_XC_n[i] + term_nipg_n * gradpsi_XC_n[i] * -XC_n) * factor );
  // //       }
  // //     }tmp=0.;
  // //     // derivative of Salt comp. wrt Sh
  // //     for (size_type j = 0; j < lfsu_Sh_s.size(); j++)
  // //     {
  // //       tmp =  0.5 * Xc_diff_m * rho_w_s * DC_w_s * gradu_XC_s * (-double(phi_Sh_s[j])) * n_F_local;
  // //       auto term_nipg_s = 0.5 * Xc_diff_m * theta_x * rho_w_s * DC_w_s * (-double(phi_Sh_s[j])) * n_F_local;
  // //       for (size_type i = 0; i < lfsv_XC_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_XC_s, i, lfsu_Sh_s, j, (tmp * psi_XC_s[i] + term_nipg_s * gradpsi_XC_s[i] * XC_s) * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_XC_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_XC_n, i, lfsu_Sh_s, j, (tmp * -psi_XC_n[i] + term_nipg_s * gradpsi_XC_s[i] * -XC_n) * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_Sh_n.size(); j++)
  // //     {
  // //       tmp =  0.5 * Xc_diff_m * rho_w_n * DC_w_n * gradu_XC_n * (-double(phi_Sh_n[j])) * n_F_local;
  // //       auto term_nipg_n = 0.5 * Xc_diff_m * theta_x * rho_w_n * DC_w_n * (-double(phi_Sh_n[j])) * n_F_local;
  // //       for (size_type i = 0; i < lfsv_XC_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_XC_s, i, lfsu_Sh_n, j, (tmp * psi_XC_s[i] + term_nipg_n * gradpsi_XC_n[i] * XC_s) * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_XC_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_XC_n, i, lfsu_Sh_n, j, (tmp * -psi_XC_n[i] + term_nipg_n * gradpsi_XC_n[i] * -XC_n) * factor );
  // //       }
  // //     }tmp=0.;
      
  // //     // derivative of Salt comp. wrt XC
  // //     for (size_type j = 0; j < lfsu_XC_s.size(); j++)
  // //     {
  // //       tmp =  - Xc_conv_m * omegaup_w_s * rho_w_s * krW_s * ( double(phi_XC_s[j])) * (Kgradu_Pw_s - rho_w_s * Kg_s) * n_F_local;
  // //       tmp +=  0.5 * Xc_diff_m * (rho_w_s * DC_w_s * Sw_s ) * (gradphi_XC_s[j]) * n_F_local; 
  // //       tmp += penalty_factor_x * phi_XC_s[j]; // Penalty term
  // //       auto term_nipg_s = 0.5 * Xc_diff_m * theta_x * rho_w_s * DC_w_n * Sw_s * double(phi_XC_s[j]) * n_F_local;
  // //       auto term_nipg_n = 0.5 * Xc_diff_m * theta_x * rho_w_n * DC_w_n * Sw_n * double(phi_XC_s[j]) * n_F_local;  
  // //       for (size_type i = 0; i < lfsv_XC_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_XC_s, i, lfsu_XC_s, j, (tmp * psi_XC_s[i] + term_nipg_s * gradpsi_XC_s[i] + term_nipg_n * gradpsi_XC_n[i])* factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_XC_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_XC_n, i, lfsu_XC_s, j, tmp * -psi_XC_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_XC_n.size(); j++)
  // //     {
  // //       tmp =  - Xc_conv_m * omegaup_w_n * rho_w_n * krW_n * ( double(phi_XC_n[j])) * (Kgradu_Pw_n - rho_w_n * Kg_n)  * n_F_local; 
  // //       tmp +=  0.5 * Xc_diff_m * (rho_w_n * DC_w_n * Sw_n ) * (gradphi_XC_n[j]) * n_F_local;
  // //       tmp += penalty_factor_x * -phi_XC_n[j]; // Penalty term
  // //       auto term_nipg_s = 0.5 * Xc_diff_m * theta_x * rho_w_s * DC_w_n * Sw_s * -double(phi_XC_n[j]) * n_F_local;
  // //       auto term_nipg_n = 0.5 * Xc_diff_m * theta_x * rho_w_n * DC_w_n * Sw_n * -double(phi_XC_n[j]) * n_F_local; 
  // //       for (size_type i = 0; i < lfsv_XC_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_XC_s, i, lfsu_XC_n, j, tmp * psi_XC_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_XC_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_XC_n, i, lfsu_XC_n, j, (tmp * -psi_XC_n[i] + term_nipg_s * gradpsi_XC_s[i] + term_nipg_n * gradpsi_XC_n[i]) * factor );
  // //       }
  // //     }tmp=0.;
  // //     // derivative of Salt comp. wrt Pw
  // //     for (size_type j = 0; j < lfsu_Pw_s.size(); j++)
  // //     {
  // //         tmp =  - Xc_conv_m * omegaup_w_s * rho_w_s * krW_s * XC_s * Kgradphi_Pw_s[j] * n_F_local; 
  // //       for (size_type i = 0; i < lfsv_XC_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_XC_s, i, lfsu_Pw_s, j, tmp * psi_XC_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_XC_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_XC_n, i, lfsu_Pw_s, j, tmp * -psi_XC_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_Pw_n.size(); j++)
  // //     {
  // //        tmp =  - Xc_conv_m * omegaup_w_n * rho_w_n * krW_n * XC_n * Kgradphi_Pw_n[j] * n_F_local; 
  // //       for (size_type i = 0; i < lfsv_XC_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_XC_s, i, lfsu_Pw_n, j, tmp * psi_XC_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_XC_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_XC_n, i, lfsu_Pw_n, j, tmp * -psi_XC_n[i] * factor );
  // //       }
  // //     }
  ...
  // //     // derivative of NCP1 comp. wrt YH2O
  // //      tmp=0.;
  // //     for (size_type j = 0; j < lfsu_YH2O_s.size(); j++)
  // //     {
  // //         tmp = penalty_factor_x * phi_YH2O_s[j]; // Penalty term
  // //       for (size_type i = 0; i < lfsv_YH2O_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_YH2O_s, i, lfsu_YH2O_s, j, tmp * psi_YH2O_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_YH2O_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_YH2O_n, i, lfsu_YH2O_s, j, tmp * -psi_YH2O_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_YH2O_n.size(); j++)
  // //     {
  // //        tmp = penalty_factor_x * -phi_YH2O_n[j]; // Penalty term
  // //       for (size_type i = 0; i < lfsv_YH2O_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_YH2O_s, i, lfsu_YH2O_n, j, tmp * psi_YH2O_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_YH2O_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_YH2O_n, i, lfsu_YH2O_n, j, tmp * -psi_YH2O_n[i] * factor );
  // //       }
  // //     }
  // //   // =============================================================== End NCP1 ==============================================================
  ...
  // //     // derivative of NCP2 comp. wrt XCH4
  // //      tmp=0.;
  // //     for (size_type j = 0; j < lfsu_XCH4_s.size(); j++)
  // //     {
  // //         tmp = penalty_factor_x * phi_XCH4_s[j]; // Penalty term
  // //       for (size_type i = 0; i < lfsv_XCH4_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_XCH4_s, i, lfsu_XCH4_s, j, tmp * psi_XCH4_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_XCH4_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_XCH4_n, i, lfsu_XCH4_s, j, tmp * -psi_XCH4_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_XCH4_n.size(); j++)
  // //     {
  // //        tmp = penalty_factor_x * -phi_XCH4_n[j]; // Penalty term
  // //       for (size_type i = 0; i < lfsv_XCH4_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_XCH4_s, i, lfsu_XCH4_n, j, tmp * psi_XCH4_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_XCH4_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_XCH4_n, i, lfsu_XCH4_n, j, tmp * -psi_XCH4_n[i] * factor );
  // //       }
  // //     }
  // //   // =============================================================== End NCP2 ==============================================================
  // //   // =================================================================== Sh penalty ========================================================

  // //     // derivative of hydrate comp. wrt Sh
  // //      tmp=0.;
  // //     for (size_type j = 0; j < lfsu_Sh_s.size(); j++)
  // //     {
  // //         tmp = penalty_factor_x * phi_Sh_s[j]; // Penalty term
  // //       for (size_type i = 0; i < lfsv_Sh_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_Sh_s, i, lfsu_Sh_s, j, tmp * psi_Sh_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Sh_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_Sh_n, i, lfsu_Sh_s, j, tmp * -psi_Sh_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_Sh_n.size(); j++)
  // //     {
  // //        tmp = penalty_factor_x * -phi_Sh_n[j]; // Penalty term
  // //       for (size_type i = 0; i < lfsv_Sh_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_Sh_s, i, lfsu_Sh_n, j, tmp * psi_Sh_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_Sh_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_Sh_n, i, lfsu_Sh_n, j, tmp * -psi_Sh_n[i] * factor );
  // //       }
  // //     }
  // //   // =============================================================== End Sh ==============================================================
  ...
  // //     // derivative of Energy comp. wrt Sg
  // //    tmp=0.;
  // //     for (size_type j = 0; j < lfsu_Sg_s.size(); j++)
  // //     {
  // //         tmp =  - Xc_conv_h * omegaup_g_s * rho_g_s * Cp_g_s * (T_s - T_ref) * krN_s * (-coeff_grad_Sw_s) * Kgradphi_Sg_s[j] * n_F_local; 
  // //       for (size_type i = 0; i < lfsv_T_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_T_s, i, lfsu_Sg_s, j, tmp * psi_T_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_T_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_T_n, i, lfsu_Sg_s, j, tmp * -psi_T_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_Sg_n.size(); j++)
  // //     {
  // //        tmp =  - Xc_conv_h * omegaup_g_n * rho_g_n * Cp_g_n * (T_n - T_ref) * krN_n *  (-coeff_grad_Sw_n) * Kgradphi_Sg_n[j] * n_F_local;
  // //       for (size_type i = 0; i < lfsv_T_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_T_s, i, lfsu_Sg_n, j, tmp * psi_T_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_T_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_T_n, i, lfsu_Sg_n, j, tmp * -psi_T_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     // derivative of Energy comp. wrt Sh
  // //     for (size_type j = 0; j < lfsu_Sh_s.size(); j++)
  // //     {
  // //         tmp =  - Xc_conv_h * omegaup_g_s * rho_g_s * Cp_g_s * (T_s - T_ref) * krN_s * (coeff_grad_Sh_s - coeff_grad_Sw_s) * Kgradphi_Sh_s[j] * n_F_local;
  // //       for (size_type i = 0; i < lfsv_T_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_T_s, i, lfsu_Sh_s, j, tmp * psi_T_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_T_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_T_n, i, lfsu_Sh_s, j, tmp * -psi_T_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_Sh_n.size(); j++)
  // //     {
  // //        tmp =  - Xc_conv_h * omegaup_g_n * rho_g_n * Cp_g_n * (T_n - T_ref) * krN_n * (coeff_grad_Sh_n - coeff_grad_Sw_n) * Kgradphi_Sh_n[j] * n_F_local;
  // //       for (size_type i = 0; i < lfsv_T_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_T_s, i, lfsu_Sh_n, j, tmp * psi_T_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_T_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_T_n, i, lfsu_Sh_n, j, tmp * -psi_T_n[i] * factor );
  // //       }
  // //     }tmp=0.;
      
  // //     // derivative of Energy comp. wrt T
  // //     for (size_type j = 0; j < lfsu_T_s.size(); j++)
  // //     {
  // //       tmp =  - Xc_conv_h * (omegaup_w_s * rho_w_s * Cp_w_s * krW_s  * (Kgradu_Pw_s - rho_w_s * Kg_s) + omegaup_g_s * rho_g_s * Cp_g_s * krN_s * (Kgradu_Pg_s - rho_g_s * Kg_s)) * double(phi_T_s[j]) * n_F_local;
  // //       tmp -=   Xc_diff_h * kth_eff * (gradphi_T_s[j]) * n_F_local; 
  // //       tmp += penalty_factor_T * phi_T_s[j]; // Penalty term
  // //       auto term_nipg_s = 0.5 * Xc_diff_h * theta_T * -kth_eff * double(phi_T_s[j]) * n_F_local;
  // //       for (size_type i = 0; i < lfsv_T_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_T_s, i, lfsu_T_s, j, (tmp * psi_T_s[i] + term_nipg_s * (gradpsi_T_s[i] + gradpsi_T_n[i]))  * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_T_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_T_n, i, lfsu_T_s, j, tmp * -psi_T_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_T_n.size(); j++)
  // //     {
  // //       tmp =  - Xc_conv_h * (omegaup_w_n * rho_w_n * Cp_w_n * krW_n  * (Kgradu_Pw_n - rho_w_n * Kg_n) + omegaup_g_n * rho_g_n * Cp_g_n * krN_n * (Kgradu_Pg_n - rho_g_n * Kg_n)) * double(phi_T_n[j]) * n_F_local; 
  // //       tmp -=   Xc_diff_h * kth_eff * (gradphi_T_n[j]) * n_F_local;
  // //       tmp += penalty_factor_T * -phi_T_n[j]; // Penalty term
  // //       auto term_nipg_s = 0.5 * Xc_diff_h * theta_T * -kth_eff * -double(phi_T_n[j]) * n_F_local;
  // //       for (size_type i = 0; i < lfsv_T_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_T_s, i, lfsu_T_n, j, tmp * psi_T_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_T_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_T_n, i, lfsu_T_n, j, (tmp * -psi_T_n[i] + term_nipg_s * (gradpsi_T_s[i] + gradpsi_T_n[i]))* factor );
  // //       }
  // //     }tmp=0.;
  // //     // derivative of Energy comp. wrt Pw
  // //     for (size_type j = 0; j < lfsu_Pw_s.size(); j++)
  // //     {
  // //         tmp =  - Xc_conv_h * (omegaup_w_s * rho_w_s * Cp_w_s * (T_s - T_ref) * krW_s + omegaup_g_s * rho_g_s * Cp_g_s * (T_s - T_ref) * krN_s ) * Kgradphi_Pw_s[j] * n_F_local; 
  // //       for (size_type i = 0; i < lfsv_T_s.size(); i++)
  // //       {
  // //           mat_ss.accumulate(lfsv_T_s, i, lfsu_Pw_s, j, tmp * psi_T_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_T_n.size(); i++)
  // //       {
  // //           mat_ns.accumulate(lfsv_T_n, i, lfsu_Pw_s, j, tmp * -psi_T_n[i] * factor );
  // //       }
  // //     }tmp=0.;
  // //     for (size_type j = 0; j < lfsu_Pw_n.size(); j++)
  // //     {
  // //        tmp =  - Xc_conv_h * (omegaup_w_n * rho_w_n * Cp_w_n * (T_n - T_ref) * krW_n + omegaup_g_n * rho_g_n * Cp_g_n * (T_n - T_ref) * krN_n ) * Kgradphi_Pw_n[j] * n_F_local; 
  // //       for (size_type i = 0; i < lfsv_T_s.size(); i++)
  // //       {
  // //           mat_sn.accumulate(lfsv_T_s, i, lfsu_Pw_n, j, tmp * psi_T_s[i] * factor );
  // //       }
  // //       for (size_type i = 0; i < lfsv_T_n.size(); i++)
  // //       {
  // //           mat_nn.accumulate(lfsv_T_n, i, lfsu_Pw_n, j, tmp * -psi_T_n[i] * factor );
  // //       }
  // //     }

  // //   // =================================================================== End Energu balance ============================================================
  // //   } //End Quadrature Rule
  // // }//End of jacobian_skeleton
  ...
  //     // auto VLE =  property.mixture.EquilibriumMoleFractions(T_n * Xc_T, Pg_n * Xc_P, XC_n, zCH4_n);
