...
public :
	...
	value ( I& intersection,/*const typename GV::Traits::template Codim<0>::Entity& element,*/
		...
		double sedimentation_depth /*m*/  = property.parameter.SedimentationDepth( globalPos[0]*property.characteristicValue.x_c );
        double grad_T = property.parameter.RegionalThermalGradient();//deg C/m

		if( property.mesh.isTopBoundary(globalPos)){
			auto icv /*ndim*/ = icvalue.evaluate(cell_inside,iplocal);
			double porosity_top = property.soil.SedimentPorosity(cell_inside,iplocal);
			double Pw_top = icv[Indices::PVId_Pw]//property.characteristicValue.P_c ;
							+ 1000.*property.parameter.g()[dim-1]* press_rate * Xc_time
							* ( time + dt ) * sedimentation_depth / property.characteristicValue.P_c ;
			...
			bcvalue[indices.PVId_T] = T_top;//property.parameter.InitialT(globalPos) ;//+ 0.035 * 0.01 * Xc_time * (time+dt) ;//+ (2/(100*365*24*3600))*time - 3/2/1000 * globalPos[0]* time;
			bcvalue[indices.PVId_C] = xc_top;//property.parameter.InitialXC(globalPos);
		}
		
		if( property.mesh.isBottomBoundary(globalPos)){
			bcvalue[indices.PVId_T] = grad_T* (property.characteristicValue.x_c/property.characteristicValue.T_c);;
		}
		// if( property.mesh.isWell(globalPos)){
